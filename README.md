...
1. Build the Docker image using the Dockerfile in the REST API entry point:
   ```bash
   docker build -t ms-tenants-rest . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Tenants.Rest/Dockerfile

   docker run -d -p 5000:5000 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-tenants-rest ms-tenants-rest
   ```

2. Build the Docker image using the Dockerfile in the gRPC entry point:
   ```bash
   docker build -t ms-tenants-grpc . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Tenants.gRpc/Dockerfile

   docker run -d -p 5001:5001 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-tenants-grpc ms-tenants-grpc
   ```

3. Build the Docker image using the Dockerfile in the Worker entry point:
   ```bash
   docker build -t ms-tenants-worker . -f src/entrypoints/CodeDesignPlus.Net.Microservice.Tenants.AsyncWorker/Dockerfile

   docker run -d -p 5002:5002 --network=backend -e ASPNETCORE_ENVIRONMENT=Docker --name ms-tenants-worker ms-tenants-worker
   ```

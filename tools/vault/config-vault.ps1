...
vault write security-codedesignplus-database/config/db-ms-tenants `
    plugin_name=mongodb-database-plugin `
    allowed_roles="ms-tenants-mongo-role" `
    connection_url="mongodb://{{username}}:{{password}}@mongo:27017/admin?ssl=false" `
    username="admin" `
    password="password"

vault write security-codedesignplus-database/roles/ms-tenants-mongo-role `
    db_name=db-ms-tenants `
    creation_statements="{ """db""": """admin""", """roles""": [{ """role""": """readWrite""", """db""": """db-ms-tenants""" }] }" `
    default_ttl="1h" `
    max_ttl="24h"

vault read security-codedesignplus-database/creds/ms-tenants-mongo-role
...
vault write security-codedesignplus-rabbitmq/roles/ms-tenants-rabbitmq-role `
    vhosts="{"""/""":{"""write""": """.*""", """read""": """.*""", """configure""": """.*"""}}"

vault read security-codedesignplus-rabbitmq/creds/ms-tenants-rabbitmq-role